...
mur = Expression(("(1+(x[0]+c1)/c2)"),c1 = 1.25,c2 = 2.5,
...
wt_interface = Expression(("(x[0]-0)*(x[0]-0)/(a*a)+(x[1]-0)*(x[1]-0)/(b*b) >= (1 - tol) && \
                          ...
                          a = 1, b = 0.75, tol = 1E-3, element = M2.ufl_element())
...
H = mur*dx(3)
DHdmu = derivative(H,mur)
DHdmu = assemble(DHdmu)
DH = Function(M2)
DH.vector()[:] = DHdmu[:] + (DHdmu[:]==0)*1
DHplot = project(DH,M2)

DHplot.vector()[:] = DJ.vector()[:]/DH.vector()[:]
#DJplot = DHplot

file4 = XDMFFile("output/DJdmu.xdmf")
file4.write(DHplot,1)


dmu = Function(M2)
dmu.vector()[:] = DJ.vector()[:]
dmu_mag = sqrt(dmu.vector().inner(dmu.vector()))
dmu.vector()[:] = (1/dmu_mag)*dmu.vector()[:] # normalizing dmu
#

mu = Function(M2)
mu.vector()[:] = mur.vector()[:]*wt.vector()[:]
h = -(0.00125/8)*sqrt(mu.vector().inner(mu.vector()))
dmu.vector()[:] = h*dmu.vector()[:]
mur.vector()[:] = mur.vector()[:] + dmu.vector()[:]
...
Rmu_h = assemble(beta*dot(grad(mur),grad(mur))*dx(3))        
Jmu_h = Rmu_h + assemble(alpha*(pr0*pr0 + pi0*pi0)*dx(4))
print("Jmu_h =", Jmu_h)


#### Finding residue ##########
Residue = abs(Jmu_h - Jmu - DJ.vector().inner(dmu.vector()))
print("Residue =",Residue)
print("del_J =", DJ.vector().inner(dmu.vector()))
